...
df_train =  pd.read_csv('sample_train.csv', nrows = 190, parse_dates=['pickup_datetime'])
...
df_train = df_train.apply(lambda r: sf.extract_time(r), axis=1)
...
normal_trip_features = ['distance_km', 'passenger_count', 'weekday', \
                         'afternoon_peak', 'morning_peak', 'night', 'normal_hour', \
                         'tolls_NJ_Manh', 'tolls_BQ_SI', 'tolls_NJ_SI', 'tolls_Manh_BQ', 'year']
...
for df in (normal_trip_X_train, normal_trip_X_validate, normal_trip_y_train, normal_trip_y_validate):
  print(df.shape)

for df in (jfk_Manhattan_X_train, jfk_Manhattan_X_validate, jfk_Manhattan_y_train, jfk_Manhattan_y_validate):
  print(df.shape)
...
normal_trip_X_train['prediction'] = normal_trip_X_train.apply(lambda row: sf.predict_fare_amount(row, metered_fare_coef_dic), axis=1)
normal_trip_X_validate['prediction'] = normal_trip_X_validate.apply(lambda row: sf.predict_fare_amount(row, metered_fare_coef_dic), axis=1)

# jfk_Manhattan_X_train['prediction'] = jfk_Manhattan_X_train.apply(lambda row: sf.predict_fare_amount(row, flat_fare_coef_dic), axis=1)
# jfk_Manhattan_X_validate['prediction'] = jfk_Manhattan_X_validate.apply(lambda row: sf.predict_fare_amount(row, flat_fare_coef_dic), axis=1)
...
df_test['distance_km'] = df_test.apply(lambda x: sf.distance(x['pickup_latitude'], x['pickup_longitude'], \
                                   x['dropoff_latitude'], x['dropoff_longitude']), axis=1)

df_test['to_airport'] = df_test.apply(lambda row: sf.airport_trip(row), axis=1)
df_airport = pd.get_dummies(df_test['to_airport'])
df_test = df_test.join(df_airport)

df_test = df_test.apply(lambda row: sf.extract_time(row), axis=1)
df_hour_type = pd.get_dummies(df_test['hour_type'])
df_test = df_test.join(df_hour_type)

df_test['tolls_NJ_Manh'] = 0
df_test['tolls_Manh_BQ'] = 0
df_test['tolls_BQ_SI'] = 0
df_test['tolls_NJ_SI'] = 0
df_test = df_test.apply(lambda row: sf.tolls_fees(row), axis=1)

# Calculate prediction of testing dataset
df_test_normal_trip = df_test[df_test['to_airport'] != 'jfk_Manhattan']
df_test_jfk_Manhattan = df_test[df_test['to_airport'] == 'jfk_Manhattan']

df_test_normal_trip['prediction'] = df_test_normal_trip.apply(lambda row: sf.predict_fare_amount(row, metered_fare_coef_dic), axis=1)
df_test_jfk_Manhattan['prediction'] = df_test_jfk_Manhattan.apply(lambda row: sf.predict_fare_amount(row, flat_fare_coef_dic), axis=1)

df_test_prediction = pd.concat([df_test_normal_trip_prediction[['key', 'prediction']], df_test_jfk_Manhattan_prediction[['key', 'prediction']]], axis=0)

print(df_test_prediction.sort_index())
